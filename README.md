...
All practice tracks sit under a single **Learning** dropdown in the nav, and the
Learning hub is the site's front page: one card per track showing where it
stands and how much is due. The nav previously carried one tab per skill, which
grew a tab every time a track was added.

The hub is generated by `tools/import-learning.js`, which is deliberately
...
The hub writes `source/index.md`, so **the home page is generated** and is
gitignored like every other importer output: a fresh checkout has no home page
until `npm run import` runs. The root previously served a second copy of the
tag tree that `/tags` already serves, so it was spending the site's most
valuable URL on a duplicate.
